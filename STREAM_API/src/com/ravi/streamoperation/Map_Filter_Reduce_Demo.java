...
public class Map_Filter_Reduce_Demo {
	...
	public static int getNoNotebooks() {
		return StudentDataBase.getStudentDetails().stream().map(Student::getNoteBook).reduce(0, Integer::sum);
	}

	// getting no. of notebook a/c to grade > 3.0 and gender is "Female"
	public static int getResult() {
		return StudentDataBase.getStudentDetails().stream().filter(student -> student.getGradeLevel() >= 3.0)
				.filter(student -> student.getGender().equals("Female")).map(Student::getNoteBook)
