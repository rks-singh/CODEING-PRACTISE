...
public class ReduceDemo {
	...
	public static void main(String[] args) {
		...
		/*
		 * Optional<com.ravi.base.Student> gpaResult = getHighestGpa(); if
		 * (gpaResult.isPresent()) { System.out.println(gpaResult.get()); }
		 */
	...
	public static int performMultiplication(List<Integer> integers) {
		...
		/*
		 * 1 step ==> a=1,b=1 form stream a*b = 1 returned 2 step ==> a=1,b=3 from
		 * stream a*b = 3 returned 3 step ==> a=3,b=5 from stream a*b = 15 returned 4
		 * step ==> a=15,b=7 from stream a*b = 105 returned ==> final result.
		 */
	}
	...
	public static Optional<Integer> performMul(List<Integer> integers) {
		return integers.stream().reduce((x, y) -> x * y);
	}

	public static Optional<Student> getHighestGpa() {
		return StudentDataBase.getStudentDetails().stream().reduce((s1, s2) -> s1.getGpa() > s2.getGpa() ? s1 : s2);
	}
}
