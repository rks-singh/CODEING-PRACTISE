...
public class SortedDemo {
	...
	public static List<String> getStudentName_I() {
		return StudentDataBase.getStudentDetails().stream().map(Student::getName).sorted().collect(Collectors.toList());
	}
	...
	public static List<String> getStudentName_D() {
		return StudentDataBase.getStudentDetails().stream().map(Student::getName)
				.sorted((s1, s2) -> s2.charAt(0) - s1.charAt(0)).collect(Collectors.toList());
	}
	...
	public static List<String> getStudentName_LD() {
		return StudentDataBase.getStudentDetails().stream().map(Student::getName)
				.sorted((s1, s2) -> s2.length() - s1.length()).collect(Collectors.toList());
	}
	...
	public static List<Student> sortStudentByGpa() {
		return StudentDataBase.getStudentDetails().stream().sorted(Comparator.comparing(Student::getGpa).reversed())
				.collect(Collectors.toList());
	}
